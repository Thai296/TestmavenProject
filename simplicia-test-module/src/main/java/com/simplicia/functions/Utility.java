...
public class Utility {
    ...
    /**
     *
     * This method will take list of items as input
     * and then pick the random item and return it
     *
     * @param lItems
     * @return
     */
    public static String getRandomItemFromList(List<String> lItems){

        Random r = new Random();

        int item = r.nextInt(lItems.size());
        String randomItem = lItems.get(item);

        return randomItem;
    }

    public static <V> V waitForElement(WebDriver driver, long timeOutInSeconds, Function<? super WebDriver, V> condition) {
        try {
            V result = new CustomWebDriverWait(driver, timeOutInSeconds).until(condition);
            if (result != null) {
                return result;
            }
            throw new MSeleniumException("Couldn't wait for " + condition.toString() + " after " + timeOutInSeconds + " seconds, got NULL", driver);
        } catch (Exception e) {
            throw new MSeleniumException("Couldn't wait for " + condition.toString() + " after " + timeOutInSeconds + " seconds", e, driver);
        }
    }
    ...
    public static String randomSentence() {
        ...
        sb.append(article[article1])
                .append(" ").append(noun[noun1])
                .append(" ").append(verb[verb1])
                .append(" ").append(preposition[preposition1])
                .append(" ").append(article[article2])
                .append(" ").append(noun[noun2])
                .append(" ").append(UUID.randomUUID().toString().replace("-", " "))
                .append(".");
